...
_.partition = function(arr, func) {
    var superArr = [];
     superArr.push(_.filter(arr,func)) + superArr.push( _.reject(arr,func));
     return superArr;
};
...
_.map = function(coll, func) {
    var newArr = [];
   _.each(coll, function(item, i, array) {
    newArr.push(func(item, i, array));
});
    return newArr;
};
/** _.pluck()
...
_.pluck = function(coll, key) {
   return  _.map(coll, function(obj,i,array) {
        return obj[key];
    });
};
...
_.reduce = function(coll, combine, seed) {
    let combined = seed;
    let i = 0;
    ...
    for(; i < coll.length; i++) {
        combined = combine(combined, coll[i], i, coll);
    }
    return combined;
};
/** _.extend()
